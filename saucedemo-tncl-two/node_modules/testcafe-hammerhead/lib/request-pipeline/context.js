...
var _createSpecialPageResponse = _interopRequireDefault(require("./create-special-page-response"));
...
class RequestPipelineContext {
  constructor(req, res, serverInfo) {
    ...
    _defineProperty(this, "temporaryCacheEntry", void 0);

    _defineProperty(this, "_injectableUserScripts", []);
  ...
  dispatch(openSessions) {
    ...
    } else this.dest.reqOrigin = this.dest.reqOrigin || this.dest.domain;
  ...
  buildContentInfo() {
    ...
    const encoding = (this.destRes.headers[_builtinHeaderNames.default.contentEncoding] || '').toLowerCase();
    ...
    _logger.default.proxy.onContentInfoBuilt(this);
  }
  ...
  async prepareInjectableUserScripts() {
    ...
    const matchedUserScripts = await Promise.all(this.session.injectable.userScripts.map(async userScript => {
      if (await userScript.page.match(requestInfo)) return userScript;
      return void 0;
    }));
    this._injectableUserScripts = matchedUserScripts.filter(userScript => !!userScript).map(userScript => userScript.url);
  }

  async _getDestResBody(res) {
    if (_incomingMessageLike.default.isIncomingMessageLike(res)) return res.getBody();
    return (0, _http.fetchBody)(this.destRes);
  }
  ...
  calculateIsDestResReadableEnded() {
    ...
    } else this.isDestResReadableEnded = true;
  }
  ...
  sendResponseHeaders() {
    ...
    const res = this.res;
    if (this.isHTMLPage && this.session.options.disablePageCaching) headerTransforms.setupPreventCachingHeaders(headers);

    _logger.default.proxy.onResponse(this, headers);
  ...
  async mockResponse() {
    _logger.default.destination.onMockedRequest(this);
  ...
  respondForSpecialPage() {
    this.destRes = (0, _createSpecialPageResponse.default)();
    this.buildContentInfo();
  }

  async fetchDestResBody() {
    this.destResBody = await this._getDestResBody(this.destRes);
    if (!this.temporaryCacheEntry) return;
  ...
  async pipeNonProcessedResponse() {
    ...
    this.destResBody = await this._getDestResBody(this.destRes);
    if (this.temporaryCacheEntry && this.destResBody.length < requestCache.MAX_SIZE_FOR_NON_PROCESSED_RESOURCE) this._addTemporaryEntryToCache();
    this.res.write(this.destResBody);
    this.res.end();
  }

  createCacheEntry(res) {
    if (requestCache.shouldCache(this) && !_incomingMessageLike.default.isIncomingMessageLike(res)) this.temporaryCacheEntry = requestCache.create(this.reqOpts, res);
  }
