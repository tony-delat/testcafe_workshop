...
function check(ctx) {
  ...
  const withCredentials = ctx.dest.credentials === _url.Credentials.include;
...
function shouldOmitCredentials(ctx) {
  switch (ctx.dest.credentials) {
    case _url.Credentials.omit:
      return true;

    case _url.Credentials.sameOrigin:
      return ctx.dest.reqOrigin !== ctx.dest.domain;

    case _url.Credentials.include:
      return false;
