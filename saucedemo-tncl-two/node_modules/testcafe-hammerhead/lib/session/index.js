...
var _defaultRequestTimeout = _interopRequireDefault(require("../request-pipeline/destination-request/default-request-timeout"));
...
class Session extends _events.EventEmitter {
  ...
  _getOptions(options = {}) {
    const requestTimeout = {
      page: options.requestTimeout && options.requestTimeout.page || _defaultRequestTimeout.default.page,
      ajax: options.requestTimeout && options.requestTimeout.ajax || _defaultRequestTimeout.default.ajax
    };
  ...
  async getRequestFilterRules(requestInfo) {
    ...
    const matchedRules = await Promise.all(rulesArray.map(async rule => {
      if (await rule.match(requestInfo)) return rule;
      return void 0;
    }));
